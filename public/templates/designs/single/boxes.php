<?php
/**
 * Enkeltkursdesign: Bokser - standard
 *
 * Basert på standard enkeltvisning, men med nytt bokslayout.
 *
 * @package kursagenten
 */
...
// Sjekk om dette er et foreldrekurs eller underkurs
...
// Hent kurskategorier til headerlenker
...
// Get instructors (brukes evt. i innhold)
 ...
 */
$render_courselist = static function ($extra_class = '') use ($all_coursedates, $price_posttext) {
    if (empty($all_coursedates)) {
        return;
    }
    ?>
    <div class="courselist <?php echo esc_attr($extra_class); ?>">
        <div class="all-coursedates">
            <h2>Kurs</h2>
            
            <div class="accordion courselist-items-wrapper expand-content" data-size="300px">
                <?php
                ...
                foreach ($all_coursedates as $index => $coursedate) :
                    ...
                    <div class="<?php echo esc_attr($item_class); ?>">
                        ...
                        <div class="accordion-content courselist-content">
                                        ...
                                        <?php endif ?>
                                        <div class="course-grid course-grid-info">
                                            <div class="content">
                                                <p>
                                                <?php if (!empty($coursedate['first_date'])): ?>
...
<article class="ka-outer-container course-container<?php echo has_post_thumbnail() ? ' has-course-image' : ''; ?>">
        ...
        <div class="ka-content-container">
            <div class="course-grid">
                ...
                <div class="content ka-box ka-highlight-background main-info-box">
                    ...
                    <a href="#ka-course-description" class="read-more-link">...les mer</a>
                </div>
                ...
                <div class="aside ka-box ka-box-background nextcourse-box">
                    <div class="nextcourse">
                        ...
                        <div class="iconlist">
                            <?php if (!empty($selected_coursedate_data['first_date'])) : ?>
                                <div>Starter: <?php echo esc_html($selected_coursedate_data['first_date']); ?></div>
                            <?php endif; ?>
                            <?php if (!empty($selected_coursedate_data['last_date'])) : ?>
                                <div>Slutter: <?php echo esc_html($selected_coursedate_data['last_date']); ?></div>
                            <?php endif; ?>
                            <?php if (!empty($selected_coursedate_data['time']) || !empty($selected_coursedate_data['course_days'])) : ?>
                                <div>Kurstider: <?php echo esc_html($selected_coursedate_data['course_days']); ?> <?php echo esc_html($selected_coursedate_data['time']); ?></div>
                            <?php endif; ?>
                            <?php if (!empty($selected_coursedate_data['duration'])) : ?>
                                <div>Varighet: <?php echo esc_html($selected_coursedate_data['duration']); ?></div>
                            <?php endif; ?>
                            <?php if (!empty($selected_coursedate_data['language'])) : ?>
                                <div>Språk: <?php echo esc_html($selected_coursedate_data['language']); ?></div>
                            <?php endif; ?>
                    ...
                    $render_courselist('courselist-tablet');
                    ?>
                </div>
            </div>
        </div>
    ...
    <!-- HOVEDINNHOLD (Bokserad 2) -->
    <section class="ka-section course-information" id="ka-course-description">
        <div class="ka-content-container">
            <div class="course-grid">
                <!-- Venstre kolonne: Bilde + Om kurset -->
                <div class="content">
                    ...
                    <?php endif; ?>
                        ...
                        <div class="ka-box ka-highlight-background course-description-box">

                        <?php
                        $has_wp_content   = !empty($wp_content);
                        $has_custom_intro = !empty($content);
                        ?>

                        <?php if ($has_wp_content || $has_custom_intro) : ?>
                            <h2>Om kurset</h2>
                        <?php endif; ?>
                        ...
                        <?php if ($admin_view === 'true') : ?>
                            <div class="edit-link">
                                <a href="<?php echo esc_url(get_edit_post_link()); ?>">
                                    ...
                                    <span class="edit-text">Legg til ekstra Wordpress innhold</span>
                                </a>
                            </div>
                        <?php endif; ?>
                        <?php if ($has_wp_content) : ?>
                            <?php if ($admin_view === 'true') : ?>
                                <div class="edit-link">
                                    <a href="<?php echo esc_url(get_edit_post_link()); ?>">
                                        <i class="ka-icon icon-edit"></i>
                                        <span class="edit-text">Rediger Wordpress innhold</span>
                                    </a>
                                </div>
                            <?php endif; ?>
                            <div class="content-text<?php echo esc_attr($admin_view_class); ?>">
                                <?php echo apply_filters('the_content', $wp_content); ?>
                            </div>
                        <?php else : ?>
                ...
                <!-- Høyre kolonne: Kursliste + Kontaktinfo -->
                <div class="aside">
                    <div class="ka-box ka-highlight-background desktop-only courselist-box">
                        <?php
                        $render_courselist('courselist-main');
                        ?>
                    </div>

                    <?php if (!empty($contact_name) || !empty($contact_phone) || !empty($contact_email)) : ?>
                        <aside class="aside">
                            <?php do_action('ka_singel_aside_before'); ?>
                            <div class="contact-info ka-box ka-highlight-background contact-info-box">
                            ...
                            <div class="similar-courses  ka-box ka-highlight-background similar-courses-box">

                                <!-- Kurs i samme kategori -->
                                    <div class="ka-content-container title-section">
                                        <h3>Kurs i samme kategori</h3>
                                        <?php echo do_shortcode('[kurs-i-samme-kategori overskrift="h4" layout="liste" grid="1" gridtablet="1" gridmobil="1" bildestr="0" fontmin="13px" fontmaks="15px" avstand="2em 0" radavstand="1em"]'); ?>
                                    </div>
                            </div>
    ...
    <!-- Kurs i samme kategori -->
    <section class="ka-section ka-footer ka-highlight-background" style="display: none;">
        <div class="ka-content-container title-section">
        </div>
    </section>
